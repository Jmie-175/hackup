...
async def explain_detection(content: str, signals: list, score: int) -> dict:
    ...
    system = (
        "You are an elite senior cybersecurity analyst specialized in advanced phishing and social engineering detection. "
        "Your mission is to protect users at all costs. You have an extremely high sensitivity to red flags. "
        "You treat any subtle indicator — such as slightly unusual wording, indirect brand mentions, or generic greetings — as potentially malicious. "
        "Never dismiss a risk as a 'false positive' unless there is absolute, undeniable evidence of legitimacy."
    )
    ...
    prompt = f"""Conduct a deep forensic analysis of this email for phishing/social engineering risk.
