...
from models.scan_models import ScanRequest, ScanResult, AttachmentResult
...
async def scan(request: ScanRequest, db: AsyncSession = Depends(get_db)):
    ...
    if request.input_type == "attachment_base64":
        ...
        # Deep static analysis for attachments whose content was fetched by the extension
        if request.attachment_files:
            for att_file in request.attachment_files:
                static_signals = analyse_attachment_b64(att_file.filename, att_file.content_b64)
                att_score  = compute_score(static_signals)
                att_verdict = get_verdict(att_score)
                ext = att_file.filename.rsplit(".", 1)[-1] if "." in att_file.filename else "bin"
                att_result = AttachmentResult(
                    filename=att_file.filename,
                    risk_score=att_score,
                    verdict=att_verdict,
                    file_type=att_file.mime_type or f".{ext}",
                    signals=static_signals,
                    macro_detected=any(
                        "macro" in s.name.lower() and s.score > 0 for s in static_signals
                    ),
                    js_detected=any(
                        "js" in s.name.lower() and s.score > 0 for s in static_signals
                    ),
                    extension_spoofed=any(
                        "double" in s.name.lower() and s.score > 0 for s in static_signals
                    ),
                )
                attachment_results.append(att_result)
                if att_score >= 40:
                    all_signals.extend(static_signals)

        # Metadata-only scoring for any remaining attachments not fetched (fallback)
        if request.attachments:
            fetched_names = {af.filename for af in (request.attachment_files or [])}
            meta_only = [a for a in request.attachments if a.get("filename") not in fetched_names]
            if meta_only:
                meta_results = score_attachments_from_dom(meta_only)
                attachment_results.extend(meta_results)
                for att in meta_results:
                    if att.risk_score >= 40:
                        all_signals.extend(att.signals)
        elif not request.attachment_files:
            # No file content and no metadata — nothing to score
            pass
