...
def score_attachment_metadata(
    ...
    # Composite score calculation (Accumulative, matching risk_engine philosophy)
    if signals:
        active_scores = [s.score for s in signals if s.score >= 30]
        if not active_scores:
            # Fallback to a weighted mean if only very low signals exist
            risk_score = int(sum(s.score for s in signals) / len(signals) * 1.2)
        else:
            # Max signal + 25% of other active signals
            max_val = max(active_scores)
            others = sorted(active_scores, reverse=True)[1:]
            # Accumulate secondary threats instead of averaging them
            risk_score = max_val + sum(w * 0.25 for w in others)

            # Safety floors for high-confidence indicators
            if max_val >= 80:
                risk_score = max(risk_score, 75)
            elif ext in MEDIUM_RISK_EXT and any(s.name == "Filename pattern" and s.score >= 35 for s in signals):
                # Specific case: suspicious document filename -> force Suspicious verdict (60+)
